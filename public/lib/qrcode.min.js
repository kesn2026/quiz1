/**
 * Compact QRCode Generator for JavaScript
 * Lightweight, zero-dependency canvas/image QR Code renderer
 */
(function(root, factory) {
      ...
      if (this.typeNumber < 1) {
        ...
        for (typeNumber = 1; typeNumber < 40; typeNumber++) {
          ...
          for (var i = 0; i < this.dataList.length; i++) {
            ...
            buffer.put(data.getLength(), typeNumber > 9 ? (typeNumber > 26 ? 16 : 16) : 8);
            data.write(buffer);
          }
  ...
  QRCodeModel.createData = function(typeNumber, errorCorrectLevel, dataList) {
    ...
    var totalDataCount = 0;
    for (var i = 0; i < rsBlocks.length; i++) totalDataCount += rsBlocks[i].dataCount;
    if (buffer.getLengthInBits() > totalDataCount * 8) throw new Error("code length overflow. (" + buffer.getLengthInBits() + ">" + totalDataCount * 8 + "bit)");
  ...
  function QRCode(targetElement, options) {
    ...
    this._el = (typeof targetElement === 'string') ? document.getElementById(targetElement) : targetElement;
    this._opts = opts;
  ...
  QRCode.prototype.makeCode = function(text) {
    if (!this._el) return;
    this._el.innerHTML = '';
    var qr = new QRCodeModel(0, this._opts.correctLevel || 1);
    qr.addData(text);
    qr.make();

    var canvas = document.createElement('canvas');
    var size = this._opts.width || 256;
    canvas.width = size;
    canvas.height = size;
    ...
    ctx.fillStyle = this._opts.colorLight || '#ffffff';
    ctx.fillRect(0, 0, size, size);

    ctx.fillStyle = this._opts.colorDark || '#000000';
    for (var row = 0; row < count; row++) {
      for (var col = 0; col < count; col++) {
        if (qr.isDark(row, col)) {
          var x = Math.round(col * tileW);
          var y = Math.round(row * tileH);
          var w = Math.ceil((col + 1) * tileW) - x;
          var h = Math.ceil((row + 1) * tileH) - y;
